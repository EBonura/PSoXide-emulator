...
| TTY and CD-ROM devices, kernel CD-ROM driver (SetMode, Setloc, ReadN, DMA per sector, Pause), kernel CD IRQ handlers and events, _96_init A(54h)/A(71h) reinstalling them, LoadTest, Load, LoadExec | `hle_files.rs`, `hle_bios.rs` | psx-spx "BIOS CDROM Functions" and "BIOS File Execute"; OpenBIOS `cdrom/` (MIT; `initCDRom` for what _96_init sets up) |
| Pad driver: InitPAD2, StartPAD2, StopPAD2, PAD_init2, PAD_dr, the priority-2 VBlank pad/card handler, the reader's byte sequence, delays and ACK timeout, effect of `_patch_pad`, `_remove_ChgclrPAD`, `_send_pad`; StartPAD2, StopPAD2, PAD_init2, StartCARD2 and StopCARD2 leave the critical section when done | `hle_pad.rs`, `hle_bios.rs` | psx-spx "BIOS Joypad Functions", "Priority Chains" and the pad patch notes; byte sequence, timeouts and the critical-section exits from OpenBIOS `sio0/driver.c` and `sio0/pad.c` (MIT). Delay and timeout lengths in cycles are estimates until measured (plan P7). |
...
| Krom2RawAdd B(51h), Krom2Offset B(53h): cell format (16x15, 1 bpp, 30 bytes), the two code ranges, -1 outside them, the ROM address 0xBFC66000 games receive | `hle_font.rs` | psx-spx B(51h)/B(53h); OpenBIOS `charset/sjis.c` (MIT) for the address and the range-table lookup |
| Font layout: bank 1 = the assigned cells of JIS X 0208 rows 1-8 in chart order (524 cells), bank 2 = level-1 kanji rows 16-47 at 94 cells a row | `hle_font.rs` | derived from the JIS X 0208 code chart (the run starts it produces were checked against OpenBIOS's lookup table, which they match) |
| Glyphs | `hle_font_glyphs.txt` | original: every glyph drawn by hand for PSoXide in that file (so far the space, full-width digits and Latin letters, which is what Chrono Cross asks for); undrawn cells show an outlined box. No Sony font data and no other font's bitmaps |
| Cycle cost of TestEvent B(0Bh) through the vector: 43 cycles for a busy event, 48 for a ready one; memcpy A(2Ah) 243 + 202.5 per byte, memset A(2Bh) 314 + 132 per byte, bzero A(28h) 322 + 132 per byte (every other call still costs 2) | `hle_bios.rs` (`call_cycles`), `cpu.rs` | black-box timing of the developer's own BIOS in the emulator: cycles from the vector to the return address; TestEvent is the median over Resident Evil 2's run (764,969 busy and 115 ready calls), the copies are fits to the fastest call of each length over Resident Evil 2, CTR, Tekken 3, WipEout, Crash and Metal Gear Solid (lengths 1 to 13,784 bytes). Resident Evil 2 and 3 time out a memory card load after 250,000 rounds of four TestEvents. |
...
- Cycle costs of the kernel routines (pad reader delays, handler
  overhead, every call but TestEvent, memcpy, memset and bzero) are
  estimates until plan phase P7 measures them. Measured retail values for
  P7: DeliverEvent about 555, UnDeliverEvent about 390, _card_read 117,
  _card_chan 65, toupper 420, strcpy and strlen about 960 to 1840 for
  short strings; the HLE charges 2. With these costs the HLE still runs
  11 to 17 frames ahead of the real kernel after the first 300 frames of
  CTR, Tekken 3 and WipEout (no input, per-frame display hashes), so
  more of the early loading path is cheaper than on a console.
